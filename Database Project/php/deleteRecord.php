...
<body>
    ...
    try {
        $sql = "DELETE FROM " . $dbname . ".Customer_Details WHERE Customer_Name = :Customer_Name";
        $stmnt = $conn->prepare($sql);     // read about prepared statement here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':Customer_Name', $_POST['Customer_Name']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
